...
// Per-turn cap on the number of non-image files we pull. Discord lets
// users drop up to 10 attachments per message; 5 is a comfortable upper
// bound that prevents a single turn from spamming /tmp while still
// covering the realistic "drop a few logs / screenshots / configs at
// once" case.
const MAX_NONIMAGE_ATTACHMENTS_PER_TURN: usize = 5;
...
// Download non-image attachments from a Discord attachments array to /tmp
// and return their absolute paths in order. Caller surfaces them as
// `attachment_file_paths` on the inbound channel tag so Mimi can Read them.
//
// Skips individual attachments silently when:
//   - the url is missing,
//   - the attachment is an image (those go through the inline-base64
//     path already),
//   - it's larger than MAX_ATTACHMENT_BYTES,
//   - or the fetch / write fails (we'd rather drop the attachment than
//     crash the turn).
//
// Returns an empty vec when no usable non-image attachments are present.
// Caps the result at MAX_NONIMAGE_ATTACHMENTS_PER_TURN to keep one chatty
// message from spamming /tmp.
async fn download_nonimage_attachments(
    ...
    for a in arr {
        if out.len() >= MAX_NONIMAGE_ATTACHMENTS_PER_TURN {
            eprintln!(
                "discord: hit {MAX_NONIMAGE_ATTACHMENTS_PER_TURN}-attachment per-turn cap, skipping remainder"
            );
            break;
        }
...
async fn run_gateway(
    ...
    *gateway_hooks::OUT_TX.write().await = Some(out_tx);
    ...
    while let Some(msg) = read.next().await {
        ...
        // Non-image attachments — pulled to /tmp and surfaced as a
        // comma-separated `attachment_file_paths` list on the inbound
        // channel tag so Mimi can Read each file directly. Capped at
        ...
        let attachment_marker = match attachment_file_paths.len() {
            ...
            n => format!(
                "\n[{n} attachments available at {} — Read them for content]",
