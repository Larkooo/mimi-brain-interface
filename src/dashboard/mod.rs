...
fn systemctl_user(args: &[&str]) -> Result<String, String> {
    ...
    if !out.status.success() {
        ...
        let msg = if !stderr.is_empty() { stderr }
                  else if !stdout.is_empty() { stdout }
                  else { format!("systemctl exited {}", out.status.code().unwrap_or(-1)) };
        return Err(msg);
    }
...
fn service_info(name: &str) -> ServiceInfo {
    ...
    // `systemctl is-enabled` exits non-zero for disabled/static/linked units
    // but still prints the state to stdout — capture both paths.
    let enabled = match systemctl_user(&["is-enabled", name]) {
        Ok(s) => s.trim() == "enabled",
        Err(s) => s.trim() == "enabled",
    };
    ...
    -> Result<Json<serde_json::Value>, (StatusCode, String)>
{
    ...
    systemctl_user(&["start", &name])
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("systemctl start {name} failed: {e}")))?;
    Ok(Json(serde_json::json!({ "ok": true })))
}
    ...
    -> Result<Json<serde_json::Value>, (StatusCode, String)>
{
    ...
    systemctl_user(&["stop", &name])
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("systemctl stop {name} failed: {e}")))?;
    Ok(Json(serde_json::json!({ "ok": true })))
}
    ...
    -> Result<Json<serde_json::Value>, (StatusCode, String)>
{
    ...
    systemctl_user(&["restart", &name])
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("systemctl restart {name} failed: {e}")))?;
    Ok(Json(serde_json::json!({ "ok": true })))
}
