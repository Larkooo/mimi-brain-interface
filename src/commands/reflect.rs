...
pub fn run() {
    ...
    const MSG: &str = "fresh context after nightly reflect 🌀";
    if let Some(chan) = latest_channel::<u64>("discord") {
        let _ = discord::write_restart_marker(chan, Some(MSG));
    }
    if let Some(chan) = latest_channel::<i64>("telegram") {
        let _ = telegram::write_restart_marker(chan, Some(MSG));
    }
...
/// context buffer, parsed as `T`. Returns `None` if nothing recent or the id
/// can't be parsed (e.g. parsing a Telegram group id as `u64` — those are
/// negative — would silently drop the marker, so callers pick the right T
/// per channel: `u64` for Discord, `i64` for Telegram).
fn latest_channel<T: FromStr>(source: &str) -> Option<T> {
