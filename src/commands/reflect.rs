...
pub fn run() {
    ...
    let restart_msg = "fresh context after nightly reflect 🌀";
    if let Some(chan) = latest_channel::<u64>("discord") {
        let _ = discord::write_restart_marker(chan, Some(restart_msg));
    }
    if let Some(chat) = latest_channel::<i64>("telegram") {
        let _ = telegram::write_restart_marker(chat, Some(restart_msg));
    }
...
/// into the requested integer type. Telegram group chat ids are negative, so
/// callers parameterize with `i64`; Discord channel ids fit in `u64`.
